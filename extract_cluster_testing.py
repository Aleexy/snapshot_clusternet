...
parser.add_argument('--model_path', default='log/clusternet.ckpt', help='model checkpoint file path [default: log/model.ckpt]')
...
def eval_one_epoch(sess, ops, num_votes=12, topk=1):
    ...
    label_f =  open(BASE_DIR+'/cluster_features/label.txt', 'w')
    labels = labels[0:num_batches*BATCH_SIZE]
    np.savetxt(label_f, labels, fmt='%d')

    for vote_idx in range(num_votes):
        log_string('vote: %d' % (vote_idx))
        feature_f = open(BASE_DIR+'/cluster_features/feature_'+ str(vote_idx) +'.txt', 'w')
