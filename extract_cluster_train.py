...
parser.add_argument('--num_point', type=int, default=996, help='Point Number [256/512/1024/2048] [default: 488]')
...
def eval_one_epoch(sess, ops, num_votes=12, topk=1):
    ...
    label_f =  open(BASE_DIR+'/cluster_features/train_label.txt', 'w+')
    labels = labels[0:num_batches*BATCH_SIZE]
    np.savetxt(label_f, labels, fmt='%d')

    for vote_idx in range(num_votes):
        log_string('vote: %d' % (vote_idx))
        feature_f = open(BASE_DIR+'/cluster_features/train_feature_'+ str(vote_idx) +'.txt', 'w')
