...
parser.add_argument('--num_point', type=int, default=448, help='Point Number [256/512/1024/2048] [default: 488]')
parser.add_argument('--model_path', default='log/contrastnet.ckpt', help='model checkpoint file path [default: log/model.ckpt]')
...
def eval_one_epoch(sess, ops, num_votes=12, topk=1):
    ...
    label_f =  open(BASE_DIR+'/contrast_features/train_label.txt', 'w+')
    labels = labels[0:num_batches*BATCH_SIZE]
    np.savetxt(label_f, labels, fmt='%d')

    for vote_idx in range(num_votes):
        log_string('vote: %d' % (vote_idx))
        feature_f = open(BASE_DIR+'/contrast_features/train_feature_'+ str(vote_idx) +'.txt', 'w')
