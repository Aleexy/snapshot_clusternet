...
parser.add_argument('--num_point', type=int, default=996, help='Point Number [256/512/1024/2048] [default: 1024]')
...
parser.add_argument('--model_path', default='log/cluster_model.ckpt', help='model checkpoint file path [default: log/model.ckpt]')
...
def train():
    with tf.Graph().as_default():
        ...
        save_path = saver.save(sess, os.path.join(LOG_DIR, 'clusternet.ckpt'))
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for fn in range(len(LABELS)):
        ...
        idx = np.random.randint(cut1.shape[0], size=int(NUM_POINT/2))
        cut1 = cut1[idx,:]
        idx = np.random.randint(cut2.shape[0], size=int(NUM_POINT/2))
        cut2 = cut2[idx,:]
